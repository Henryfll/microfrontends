# Pipeline CI/CD - MF-A (Module Federation)

Pipeline experimental reproducible para el microfrontend **MF-A**.
Archivo: [`.github/workflows/mf-a-pipeline.yml`](mf-a-pipeline.yml).

> Este pipeline construye, despliega y valida **únicamente** MF-A.
> Host, MF-B y MF-C tienen sus propios workflows equivalentes.
...
| Puerto             | `4201`                      |
| Imagen Docker      | `mf-a-mf:latest`            |
| Node.js            | `24.13.0` (fija)            |
| npm                | `11.6.2`                    |
| Angular            | `21.1.0`                    |
| TypeScript         | `5.9.2` (compatible Ng 21)  |
| Webpack            | `5.106.2`                   |
...
| `COMPONENT_NAME`   | Identificador del microfrontend (`mf-a`).                             |
| `COMPONENT_ROLE`   | Rol del componente dentro de Module Federation (`mf-a`).              |
| `ORCHESTRATOR`     | `module-federation`. Se mantiene igual al variar a `single-spa`.      |
| `K8S_NAMESPACE`    | Namespace destino (`mf-experiment`).                                  |
| `K8S_DEPLOYMENT`   | Nombre del Deployment a esperar (`mf-a`).                             |
| `K8S_SERVICE`      | Nombre del Service expuesto (`mf-a`).                                 |
| `APP_PORT`         | Puerto asignado al microfrontend (`4201`).                            |
| `SMOKE_URL`        | URL del smoke test (`http://localhost:4201`).                         |
| `DOCKER_IMAGE`     | Nombre de imagen (`mf-a-mf`).                                         |
| `DOCKER_TAG`       | Tag de imagen (`latest`).                                             |
| `COMPONENT_DIR`    | Directorio del microfrontend en el monorepo (`mf-a`).                 |
| `K8S_DIR`          | Directorio de manifiestos K8s (`mf-a/k8s`).                           |
...
| 8  | Smoke test HTTP `localhost:4201`  | `Smoke test (HTTP)`            | `smoke_result`   |
...
El archivo se publica como artefacto `metrics-mf-a-<run_id>` (retención 30 días)
...
export COMPONENT_DIR=mf-a
export K8S_DIR=mf-a/k8s
export K8S_NAMESPACE=mf-experiment
export K8S_DEPLOYMENT=mf-a
export K8S_SERVICE=mf-a
export DOCKER_IMAGE=mf-a-mf
...
- Poder leer la imagen `mf-a-mf:latest` desde el daemon local
  (la imagen se construye en el mismo runner, por eso `imagePullPolicy: IfNotPresent`).
- Tener libre el puerto `4201` durante el `port-forward` del smoke test.
...
| No optimizar el build para frameworks específicos              | Se invoca `ng build` vía `npm run build`, sin flags adicionales.          |
...
| Service expuesto en el puerto asignado                         | `port-forward svc/mf-a 4201:4201` -> `http://localhost:4201`.             |
| Una corrida = un microfrontend                                 | El workflow solo opera sobre `mf-a/`.                                     |
...
- **Push**: solo cuando cambian `mf-a/**` o el propio workflow, en la rama
