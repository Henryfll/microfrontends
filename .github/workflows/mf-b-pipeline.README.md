# Pipeline CI/CD - MF-B (Module Federation)

Pipeline experimental reproducible para el microfrontend **MF-B** (React 19).
Archivo: [`.github/workflows/mf-b-pipeline.yml`](mf-b-pipeline.yml).

> Este pipeline construye, despliega y valida **únicamente** MF-B.
> Host, MF-A y MF-C tienen sus propios workflows equivalentes.
...
| Puerto             | `4202`                      |
| Imagen Docker      | `mf-b-mf:latest`            |
| Node.js            | `24.13.0` (fija)            |
| npm                | `11.6.2`                    |
| React / ReactDOM   | `19.2.0`                    |
| TypeScript         | `~5.9.2` (compatible React 19) |
| Webpack            | `5.x`                       |
...
| `COMPONENT_NAME`   | Identificador del microfrontend (`mf-b`).                             |
| `COMPONENT_ROLE`   | Rol del componente dentro de Module Federation (`mf-b`).              |
| `ORCHESTRATOR`     | `module-federation`. Se mantiene igual al variar a `single-spa`.      |
| `K8S_NAMESPACE`    | Namespace destino (`mf-experiment`).                                  |
| `K8S_DEPLOYMENT`   | Nombre del Deployment a esperar (`mf-b`).                             |
| `K8S_SERVICE`      | Nombre del Service expuesto (`mf-b`).                                 |
| `APP_PORT`         | Puerto asignado al microfrontend (`4202`).                            |
| `SMOKE_URL`        | URL del smoke test (`http://localhost:4202`).                         |
| `DOCKER_IMAGE`     | Nombre de imagen (`mf-b-mf`).                                         |
| `DOCKER_TAG`       | Tag de imagen (`latest`).                                             |
| `COMPONENT_DIR`    | Directorio del microfrontend en el monorepo (`mf-b`).                 |
| `K8S_DIR`          | Directorio de manifiestos K8s (`mf-b/k8s`).                           |
...
| 8  | Smoke test HTTP `localhost:4202`  | `Smoke test (HTTP)`            | `smoke_result`   |
...
El archivo se publica como artefacto `metrics-mf-b-<run_id>` (retención 30 días)
...
export COMPONENT_DIR=mf-b
export K8S_DIR=mf-b/k8s
export K8S_NAMESPACE=mf-experiment
export K8S_DEPLOYMENT=mf-b
export K8S_SERVICE=mf-b
export DOCKER_IMAGE=mf-b-mf
...
- Poder leer la imagen `mf-b-mf:latest` desde el daemon local
  (la imagen se construye en el mismo runner, por eso `imagePullPolicy: IfNotPresent`).
- Tener libre el puerto `4202` durante el `port-forward` del smoke test.
...
| Service expuesto en el puerto asignado                         | `port-forward svc/mf-b 4202:4202` -> `http://localhost:4202`.             |
| Una corrida = un microfrontend                                 | El workflow solo opera sobre `mf-b/`.                                     |
...
- **Push**: solo cuando cambian `mf-b/**` o el propio workflow, en la rama
