# Pipeline CI/CD - Host (Module Federation)

Pipeline experimental reproducible para el **Host** (shell que carga MF-A/B/C).
Archivo: [`.github/workflows/host-pipeline.yml`](host-pipeline.yml).

> Este pipeline construye, despliega y valida **únicamente** el Host.
> MF-A, MF-B y MF-C tienen sus propios workflows equivalentes.
...
| Componente         | host (shell)                |
| Namespace K8s      | `mf-experiment`             |
| Puerto             | `4200`                      |
| Imagen Docker      | `mf-host:latest` (la declarada en `host/k8s/deployment.yaml`) |
| Node.js            | `24.13.0` (fija)            |
| npm                | `11.6.2`                    |
| Webpack            | `5.x`                       |
| TypeScript         | n/a (host es JS puro vía Webpack) |

> Nota: el host actual es JavaScript puro (sin TypeScript), por lo que no aplica
> regla de compatibilidad de TS. Si en una iteración futura se introduce TS,
> usar la misma versión que su contraparte (`~5.9.2` para alinear con MF-A/B).
...
| `COMPONENT_NAME`   | Identificador del microfrontend (`host`).                             |
| `COMPONENT_ROLE`   | Rol del componente dentro de Module Federation (`host`).              |
| `ORCHESTRATOR`     | `module-federation`. Se mantiene igual al variar a `single-spa`.      |
| `K8S_NAMESPACE`    | Namespace destino (`mf-experiment`).                                  |
| `K8S_DEPLOYMENT`   | Nombre del Deployment a esperar (`host`).                             |
| `K8S_SERVICE`      | Nombre del Service expuesto (`host`).                                 |
| `APP_PORT`         | Puerto asignado al microfrontend (`4200`).                            |
| `SMOKE_URL`        | URL del smoke test (`http://localhost:4200`).                         |
| `DOCKER_IMAGE`     | Nombre de imagen (`mf-host`, debe coincidir con el manifiesto K8s).   |
| `DOCKER_TAG`       | Tag de imagen (`latest`).                                             |
| `COMPONENT_DIR`    | Directorio del microfrontend en el monorepo (`host`).                 |
| `K8S_DIR`          | Directorio de manifiestos K8s (`host/k8s`).                           |
...
| 8  | Smoke test HTTP `localhost:4200`  | `Smoke test (HTTP)`            | `smoke_result`   |
...
El archivo se publica como artefacto `metrics-host-<run_id>` (retención 30 días)
...
export COMPONENT_DIR=host
export K8S_DIR=host/k8s
export K8S_NAMESPACE=mf-experiment
export K8S_DEPLOYMENT=host
export K8S_SERVICE=host
export DOCKER_IMAGE=mf-host
...
- Poder leer la imagen `mf-host:latest` desde el daemon local
  (la imagen se construye en el mismo runner, por eso `imagePullPolicy: IfNotPresent`).
- Tener libre el puerto `4200` durante el `port-forward` del smoke test.

> Recordatorio: para que el host pueda cargar a sus remotes en runtime hay que
> haber desplegado **antes** los pipelines de MF-A/B/C en el mismo namespace.
> El smoke test del host valida que el shell responde HTTP 200; la validación
> funcional cruzada (carga de remotes en el navegador) queda fuera del smoke
> test obligatorio del pipeline.
...
| Service expuesto en el puerto asignado                         | `port-forward svc/host 4200:4200` -> `http://localhost:4200`.             |
| Una corrida = un microfrontend                                 | El workflow solo opera sobre `host/`.                                     |
...
- **Push**: solo cuando cambian `host/**` o el propio workflow, en la rama
  `module-federation`. Esto evita corridas cruzadas con otros microfrontends.

---

## 9. Tabla de pipelines hermanos

Misma plantilla, solo cambian `env:`:

| Componente | Puerto | `K8S_DEPLOYMENT` / `K8S_SERVICE` | `DOCKER_IMAGE`     |
| ---------- | ------ | -------------------------------- | ------------------ |
| host       | 4200   | `host`                           | `mf-host`          |
